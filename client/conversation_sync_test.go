...
func TestSyncConversation(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if request.UserID != "hermes-user" || request.Title != "Daily chat" || request.EventID != "evt-1" || request.Text != "Q/A" {
	...
	result, err := api.SyncConversation(context.Background(), &ConversationSyncRequest{
		UserID: "hermes-user", Title: "Daily chat", Text: "Q/A", ConversationAt: &at, EventID: "evt-1",
	})
	if err != nil {
		t.Fatal(err)
	}
