...
const MARKDOWN_SAMPLE = `# 华清智言知识库知识框架

华清智言知识库是一个基于 LLM 的企业知识库平台，集 RAG 问答、ReAct 智能体、Wiki 知识图谱于一体。本文介绍其设计动机、架构与典型用法。

## 设计动机

企业内部知识散落在 Confluence、飞书、Notion、Git 仓库等多种系统，传统全文检索难以理解语义，单一 LLM 又缺少可信赖的上下文来源。华清智言知识库的目标是：
...
\`\`\`bash
cd hqzy-knowledge         # 进入管理员提供的项目目录
...
const FAQ_SAMPLE = `# 华清智言知识库部署与使用 FAQ
