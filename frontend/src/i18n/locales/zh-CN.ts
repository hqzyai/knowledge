export default {
  ...
  integrations: {
    ...
    claw: {
      title: '华清智言知识库 Skill',
      subtitle: '通过华清智言知识库 REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。',
      ...
      downloadSuccess: '华清智言知识库 Skill 下载已开始',
      downloadFailed: '下载 Skill 失败，请稍后重试',
      ecosystemNote: '下载包已写入当前知识库 API 地址，可按包内说明完成本地安装。',
      installCta: '前往 ClawHub',
      installCtaHint: '安装华清智言知识库 Skill · 将在新标签页打开',
      hubMeta: '华清智言知识库 · 本地安装包',
      steps: {
        verify: {
          title: '验证连接',
        ...
        download: {
          title: '下载并安装 Skill',
          desc: '下载上方 ZIP，解压后按包内说明安装。Skill 已写入当前 API 地址。'
        },
        env: {
          title: '配置 API Key',
          desc: '按照下载包内的说明，在 OpenClaw 运行环境中配置 API Key。'
        },
        api: {
          title: '获取 API 凭证',
    ...
    chrome: {
      title: '知识管理助手',
      subtitle: '配合华清智言知识库自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。',
      ...
      steps: {
        ...
        port: {
          title: '桌面版配置固定端口（推荐）',
          desc: '使用华清智言知识库桌面版时，在 API 信息中设置固定端口（如 37841），避免每次启动后地址变化导致插件断连。'
        },
        api: {
          title: '获取 API 凭证',
    ...
    api: {
      ...
      capabilityManageModelsHint: '允许管理模型配置、模型凭据、模型连通性测试和 云服务 凭据。',
      ...
      signedFlowDetail: 'HMAC 密钥仅保存在华清智言知识库与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=hqzy-knowledge、exp（有效期不超过 24 小时）。',
      ...
      hmacSecret: 'HMAC 密钥',
      hmacSecretDesc: '与华清智言知识库保存的同一份密钥；仅用于后端签发 JWT，不要作为请求头发送。',
      ...
      signedRequestStep0: '# 0. 在你的后端签发 JWT（HS256，aud=hqzy-knowledge，sub=用户 ID，tenant_id={tenantId}，exp 不超过 24 小时）',
  ...
  system: {
    ...
    dbMigrationFailedDesc: '启动时数据库迁移未成功完成，部分表或索引可能未创建，会导致 Wiki、知识图谱等功能异常。请联系管理员，并提供下方错误详情以便排查。',
  ...
  embedPublish: {
    ...
    webhookUrlDesc: '可选。访客发消息与助手回复完成时，华清智言知识库会向该 HTTPS 地址 POST 事件（message_sent / message_received）。',
    webhookSecret: 'Webhook 签名密钥',
    webhookSecretPlaceholder: '用于 HMAC-SHA256 签名的密钥',
  ...
  auth: {
    firstLoginPassword: {
      loadingPolicy: '正在加载密码规则…',
      title: '设置您的登录密码',
      description: '您正在使用初始密码。请先设置一个仅您知道的新密码，完成后即可继续使用。',
      policyError: '暂时无法获取密码规则，请重试。',
      otherAccount: '退出并使用其他账号',
    },
    login: '登录',
    logout: '退出',
    ...
    registerFailed: '注册失败',
    subtitle: '登录您的账户，开始探索知识',
    registerSubtitle: '创建账户，让知识触手可及',
  ...
  envVarSettings: {
    title: '沙箱密钥',
    description: '给技能和沙箱用的个人密钥，不是华清智言知识库的系统或部署配置。',
    helpAria: '沙箱密钥说明',
    ...
    nameInvalid: '这个名字不能用。沙箱保留的名字和系统内部前缀不接受。',
  ...
  settings: {
    ...
    sandbox: {
      ...
      dockerHostRisk: '留空或 unix:// 会使用华清智言知识库所在机器的 Docker 守护进程，权限等同该机 root，只适合私有化单机。多套空间共用同一主机时请改用 Cube 或 E2B。远程 tcp:// 必须填写 TLS 证书目录。',
      addConfig: '添加沙箱',
      ...
      weknoraTemplateTitle: '华清智言知识库标准模板',
      weknoraDockerImage: '华清智言知识库标准镜像',
      weknoraDockerImageHint: '每次会话独占一个长驻容器，脚本、shell 命令与文件都在同一个容器内，会话结束或空闲超时后回收。',
      weknoraTemplateOverview: '华清智言知识库负责提供标准运行环境。连接集群后会自动发现模板，缺少标准模板时自动创建。',
      weknoraTemplateDescription: '包含华清智言知识库技能所需的 Python、Node.js、CLI 工具、工作目录和非 root 执行用户。',
      recommendedTag: '推荐',
      ...
      templateLoadHint: '填写集群连接后刷新模板。没有华清智言知识库标准模板时点「创建」；改 DNS 或镜像后点「删除并重建」。',
      templateLoadFailed: '模板列表加载失败',
      standardTemplateProvisioning: '已开始在集群中创建华清智言知识库标准模板，请稍后刷新查看状态',
      ...
      templateStepHint: '进入此步只会列出集群已有模板，不会自动创建。没有标准模板时点「创建华清智言知识库标准模板」；改 DNS 或镜像后点「删除并重建」。模板就绪后才能继续。',
      loadingTemplates: '正在从集群加载模板…',
      templateBuildingHint: '标准模板正在构建，列表会自动刷新。',
      templateUntaggedHint: '构建已完成，但没有构建带 default 标签，创建沙箱时无法解析。请在 E2B 删除该模板，刷新后华清智言知识库会重新构建。',
      templateFailedReason: '构建失败：{reason}',
      noTemplates: '当前集群未返回可用模板。',
      weknoraStandardTemplate: '华清智言知识库标准模板',
      createStandardTemplate: '创建',
      createStandardTemplateHint: '按当前连接配置构建，包含 DNS。之后改配置可在卡片上重建。',
      replaceStandardTemplate: '重建',
      replaceStandardTemplateConfirm: '将用当前配置（含 DNS）重建华清智言知识库标准模板。新模板就绪前不会删除仍可启动的旧模板。',
      ...
      cubeDnsServersHelp: '可选。写入华清智言知识库标准模板的 nameserver（须为 IP）。留空则使用集群默认（常见 119.29.29.29）。私网或云上 UDP 53 出不去时，填 Cube 宿主机 /etc/resolv.conf 里能用的地址，并避开 10/8、172.16/12、192.168/16。已有标准模板需在模板卡片上点「重建」才会生效。',
      ...
      dockerTlsCertPathHelp: '华清智言知识库所在主机上包含 ca.pem、cert.pem、key.pem 的目录。远程守护进程必填，证书不入库，由部署方挂载。',
      dockerIdleTtl: '空闲回收（秒）',
      dockerIdleTtlHelp: 'Docker 守护进程本身没有空闲超时。容器多久没有执行任何命令就会被华清智言知识库回收，会话继续时重建。留空按 1800 秒。',
    ...
    weknoraCloud: {
      title: '云服务',
      description: '配置 云服务 的 APPID 和 APPSECRET 凭证。凭证用于模型服务和文档解析引擎。',
      viewDocs: '查看文档',
      ...
      addModelsDisplayName: {
        chat: '云服务 对话',
        embedding: '云服务 Embedding',
        rerank: '云服务 ReRank',
        vllm: '云服务 视觉'
      },
      modelsSection: {
        title: '云模型接入',
        descReady: '将 云服务 提供的四类标准模型注册到当前空间，用于对话、向量检索、重排序与多模态理解。',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '创建你的知识库',
        desc: '知识库是一切的起点：上传文档、网页或 FAQ，华清智言知识库会自动解析并建立索引。点击这里进入知识库。'
      },
      welcome: {
        title: '欢迎使用华清智言知识库',
