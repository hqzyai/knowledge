export default {
  ...
  integrations: {
    ...
    claw: {
      title: '华清智言知识库 Skill',
      subtitle: 'Импорт документов и гибридный поиск (вектор + ключевые слова) через REST API 华清智言知识库 — загрузки, URL, Markdown и поиск.',
      ...
      downloadSuccess: 'Загрузка 华清智言知识库 Skill началась',
      downloadFailed: 'Не удалось скачать Skill. Повторите попытку позже.',
      ecosystemNote: 'Пакет содержит текущий адрес API базы знаний. Выполните локальную установку по инструкции в пакете.',
      installCta: 'Открыть ClawHub',
      installCtaHint: 'Установка 华清智言知识库 Skill · откроется в новой вкладке',
      hubMeta: '华清智言知识库 · Пакет для локальной установки',
      steps: {
        verify: {
          title: 'Проверка',
        ...
        download: {
          ...
          desc: 'Скачайте и распакуйте ZIP, затем следуйте инструкции по установке. Адрес API уже указан.'
        },
        env: {
          title: 'Настройте API Key',
          desc: 'Настройте API-ключ в среде OpenClaw по инструкции в пакете.'
        },
        api: {
    ...
    chrome: {
      ...
      subtitle: 'Для self-hosted 华清智言知识库: вопросы в боковой панели, клиппинг страниц и Markdown-заметки в базы знаний прямо при просмотре.',
      ...
      steps: {
        ...
        port: {
          ...
          desc: 'В 华清智言知识库 Desktop задайте фиксированный порт API (например 37841) в API-информации.'
        },
        api: {
    ...
    api: {
      ...
      capabilityManageModelsHint: 'Управление конфигурациями моделей, учётными данными, проверками подключения и данными Облачный сервис.',
      ...
      hmacSecretDesc: 'Тот же secret, что в 华清智言知识库; только для подписи JWT на backend — не отправляйте его в заголовке запроса.',
      ...
      signedRequestStep0: '# 0. Подпишите JWT на backend (HS256, aud=hqzy-knowledge, sub=ID пользователя, tenant_id={tenantId}, exp не более 24 ч)',
  ...
  system: {
    ...
    dbMigrationFailedDesc: 'Миграция базы данных при запуске не завершилась. Отсутствующие таблицы или индексы могут нарушить работу Wiki и графа знаний. Передайте администратору сведения об ошибке ниже.',
  ...
  embedPublish: {
    ...
    webhookUrlPlaceholder: 'https://your-server.example.com/knowledge/embed-events',
    webhookUrlDesc: 'Optional. 华清智言知识库 POSTs message_sent / message_received events to this HTTPS endpoint.',
    webhookSecret: 'Webhook secret',
    webhookSecretPlaceholder: 'Секрет для подписи HMAC-SHA256',
  ...
  createChat: {
    title: 'Привет, я 华清智言知识库 — ваши знания всегда под рукой',
  ...
  auth: {
    firstLoginPassword: {
      loadingPolicy: 'Загрузка требований к паролю…',
      title: 'Установите пароль',
      description: 'Вы используете начальный пароль. Чтобы продолжить, задайте новый пароль, известный только вам.',
      policyError: 'Не удалось загрузить требования к паролю. Повторите попытку.',
      otherAccount: 'Выйти и использовать другую учётную запись',
    },
    login: 'Вход',
    logout: 'Выход',
    ...
    firstTime: 'Нет аккаунта?',
    registerSuccess: 'Регистрация завершена. Войдите в систему',
    registerFailed: 'Ошибка регистрации',
    subtitle: 'Войдите в аккаунт, чтобы работать со знаниями',
    registerSubtitle: 'Создайте аккаунт, чтобы начать работу',
  ...
  envVarSettings: {
    ...
    description: 'Личные ключи для навыков и песочниц, а не системные или деплой-настройки 华清智言知识库.',
    ...
    nameInvalid: 'Имя зарезервировано песочницей или использует системный префикс.',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: 'Держит долгоживущий контейнер на каждую сессию на этом хосте 华清智言知识库; скрипты и файлы остаются в том же контейнере',
      },
      ...
      dockerHostRisk: 'Пустое значение или unix:// использует демон Docker на машине 华清智言知识库 — это равносильно root на этой машине. Только для частной одноузловой установки. Если несколько пространств делят один хост, используйте Cube или E2B. Удалённый tcp:// требует каталог TLS-сертификатов.',
      ...
      weknoraTemplateTitle: '华清智言知识库 standard template',
      weknoraDockerImage: '华清智言知识库 standard image',
      ...
      weknoraTemplateOverview: '华清智言知识库 provides the standard runtime. Templates are discovered after connecting and the standard one is created when missing.',
      weknoraTemplateDescription: 'Includes the Python, Node.js, CLI tools, workspace path, and non-root execution user expected by 华清智言知识库 skills.',
      ...
      templateLoadHint: 'Введите подключение к кластеру и обновите список. Если стандартного шаблона 华清智言知识库 нет, нажмите «Создать»; после смены DNS или образа — «Удалить и пересобрать».',
      templateLoadFailed: 'Failed to load templates',
      standardTemplateProvisioning: 'The 华清智言知识库 standard template is being created. Refresh shortly to see its status.',
      ...
      templateStepHint: 'На этом шаге только перечисляются шаблоны, уже есть в кластере; ничего не создаётся автоматически. Если стандартного шаблона нет, нажмите «Создать стандартный шаблон 华清智言知识库»; после смены DNS или образа — «Удалить и пересобрать». Продолжить можно, когда шаблон готов.',
      ...
      templateUntaggedHint: 'Сборки завершены, но ни одна не имеет тега default, поэтому при создании песочницы шаблон не находится. Удалите его в E2B и обновите список — 华清智言知识库 пересоберёт шаблон.',
      templateFailedReason: 'Сборка не удалась: {reason}',
      noTemplates: 'No templates were returned by this cluster.',
      weknoraStandardTemplate: 'Стандартный шаблон 华清智言知识库',
      ...
      replaceStandardTemplateConfirm: 'Стандартный шаблон 华清智言知识库 будет собран заново с текущими настройками, включая DNS. Предыдущий готовый шаблон не удаляется, пока замена не станет готовой к запуску.',
      ...
      cubeDnsServersHelp: 'Необязательно. IP nameserver, которые записываются в стандартный шаблон 华清智言知识库. Пустое значение оставляет значение кластера по умолчанию (обычно 119.29.29.29). Если UDP/53 до публичных резолверов недоступен, укажите достижимые адреса из /etc/resolv.conf хоста Cube, исключая 10/8, 172.16/12 и 192.168/16. Чтобы изменение вступило в силу, на карточке шаблона нажмите «Пересобрать».',
      ...
      dockerTlsCertPathHelp: 'Directory on the 华清智言知识库 host holding ca.pem, cert.pem and key.pem. Required for a remote daemon; certificates are mounted by the deployment, never stored here.',
      dockerIdleTtl: 'Idle reclaim (seconds)',
      dockerIdleTtlHelp: 'The Docker daemon has no idle timeout of its own. A container that runs no command for this long is reclaimed by 华清智言知识库 and rebuilt when the session continues. Empty means 1800 seconds.',
    ...
    weknoraCloud: {
      ...
      description: 'Настройте учётные данные APPID и APPSECRET для Облачный сервис. Данные используются для модельных сервисов и движка парсинга документов.',
      ...
      expired: 'Учётные данные Облачный сервис истекли',
      ...
      appIdLabel: 'APPID',
      appIdDesc: 'ID приложения Облачный сервис',
      appIdPlaceholder: 'Введите APPID',
      appSecretLabel: 'APPSECRET',
      appSecretDesc: 'Секрет приложения Облачный сервис',
      ...
      addModelsDisplayName: {
        chat: 'Облачный сервис Chat',
        embedding: 'Облачный сервис Embedding',
        rerank: 'Облачный сервис ReRank',
        vllm: 'Облачный сервис Vision'
      },
      modelsSection: {
        title: 'Облачные модели',
        descReady: 'Зарегистрируйте четыре стандартные модели Облачный сервис для чата, поиска, реранкинга и зрения.',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        ...
        desc: 'С этого всё начинается: загружайте документы, веб-страницы или FAQ, и 华清智言知识库 автоматически разберёт и проиндексирует их. Нажмите здесь, чтобы открыть базы знаний.'
      },
      welcome: {
        title: 'Добро пожаловать в 华清智言知识库',
