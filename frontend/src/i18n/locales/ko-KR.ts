export default {
  ...
  integrations: {
    ...
    claw: {
      title: '华清智言知识库 Skill',
      subtitle: '华清智言知识库 REST API로 문서를 가져오고 하이브리드 검색(벡터+키워드)을 수행합니다. 파일/URL/Markdown 업로드 및 검색에 사용.',
      ...
      downloadCta: 'Skill 폴더 다운로드',
      downloadCtaHint: 'ZIP에 현재 API URL 포함 · API Key만 필요',
      downloadSuccess: '华清智言知识库 Skill 다운로드가 시작되었습니다',
      downloadFailed: 'Skill을 다운로드하지 못했습니다. 잠시 후 다시 시도하세요.',
      ecosystemNote: '다운로드 패키지에 현재 지식베이스 API 주소가 포함됩니다. 패키지 안내에 따라 로컬로 설치하세요.',
      installCta: 'ClawHub 열기',
      installCtaHint: '华清智言知识库 Skill 설치 · 새 탭에서 열림',
      hubMeta: '华清智言知识库 · 로컬 설치 패키지',
      steps: {
        verify: {
          title: '연결 확인',
        ...
        download: {
          title: 'Skill 다운로드 및 설치',
          desc: 'ZIP을 다운로드하고 압축을 푼 뒤 안내에 따라 설치하세요. 현재 API 주소가 포함되어 있습니다.'
        },
        env: {
          title: 'API Key 설정',
          desc: '패키지 안내에 따라 OpenClaw 환경에 API 키를 설정하세요.'
        },
        api: {
          title: 'API 자격 증명',
          desc: '설정 → API 정보에서 API Key를 만들거나 복사하세요. 현재 API URL은 다운로드 시 자동으로 포함됩니다.'
        }
      },
      capabilities: {
        browse: {
    ...
    chrome: {
      title: '지식 관리 어시스턴트',
      subtitle: '자체 호스팅 华清智言知识库와 함께: 사이드바에서 질문하고, 웹 페이지를 클리핑하며, Markdown 메모를 지식베이스에 저장하세요.',
      ...
      steps: {
        ...
        port: {
          title: '데스크톱: 고정 포트(권장)',
          desc: '华清智言知识库 데스크톱에서는 API 정보에서 고정 포트(예: 37841)를 설정하세요.'
        },
        api: {
          title: 'API 자격 증명',
    ...
    api: {
      ...
      signedFlowDetail: 'HMAC secret은 华清智言知识库와 신뢰할 수 있는 백엔드에만 보관하세요. 요청에 넣거나 브라우저에 배포하지 마세요. 요청 헤더에는 secret이 아니라 해당 secret으로 서명한 JWT 문자열을 넣습니다(발급마다 또는 만료 후 변경). JWT에는 sub(외부 사용자 ID), tenant_id, aud=hqzy-knowledge, exp(최대 24시간)가 필요합니다.',
      ...
      hmacSecretDesc: '华清智言知识库에 저장된 것과 동일한 secret. 백엔드에서 JWT 서명에만 사용하며 요청 헤더로 보내지 않습니다.',
      ...
      signedRequestStep0: '# 0. 백엔드에서 JWT 서명 (HS256, aud=hqzy-knowledge, sub=사용자 ID, tenant_id={tenantId}, exp 최대 24시간)',
  ...
  system: {
    ...
    dbMigrationFailedDesc: '시작 시 데이터베이스 마이그레이션이 완료되지 않아 Wiki 및 지식 그래프 등의 기능에 영향을 줄 수 있습니다. 아래 오류 정보를 관리자에게 전달해 주세요.',
  ...
  embedPublish: {
    ...
    webhookSecretPlaceholder: 'HMAC-SHA256 서명 키',
  ...
  createChat: {
    title: '안녕하세요, 华清智言知识库입니다 — 당신의 지식을 손끝에',
  ...
  auth: {
    firstLoginPassword: {
      loadingPolicy: '비밀번호 규칙을 불러오는 중…',
      title: '비밀번호 설정',
      description: '초기 비밀번호를 사용하고 있습니다. 계속하려면 본인만 아는 새 비밀번호를 설정해 주세요.',
      policyError: '비밀번호 규칙을 불러오지 못했습니다. 다시 시도해 주세요.',
      otherAccount: '로그아웃하고 다른 계정 사용',
    },
    login: '로그인',
    logout: '로그아웃',
    oidcLogin: 'OIDC로 로그인',
    ...
    subtitle: '계정에 로그인하고 지식을 탐색하세요',
    registerSubtitle: '계정을 만들고 시작하세요',
  ...
  envVarSettings: {
    title: '샌드박스 키',
    description: '스킬과 샌드박스에 쓰는 개인 키이며, 华清智言知识库 시스템이나 배포 설정이 아닙니다.',
    helpAria: '샌드박스 키 설명',
    ...
    nameInvalid: '샌드박스 예약 이름이나 시스템 접두사는 사용할 수 없습니다.',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: '이 华清智言知识库 호스트의 Docker에서 세션마다 장수명 컨테이너를 유지합니다. 스크립트와 파일이 같은 컨테이너에 남습니다',
      },
      ...
      dockerHostRisk: '비우거나 unix:// 를 쓰면 华清智言知识库가 있는 머신의 Docker 데몬을 사용하며, 그 권한은 해당 머신의 root와 같습니다. 프라이빗 단일 노드에만 쓰세요. 여러 워크스페이스가 같은 호스트를 쓰는 경우에는 Cube 또는 E2B를 쓰세요. 원격 tcp:// 는 TLS 인증서 디렉터리가 필요합니다.',
      addConfig: '샌드박스 추가',
      ...
      templateLoadHint: '클러스터 연결을 입력한 뒤 새로고침하세요. 华清智言知识库 표준 템플릿이 없으면 「만들기」를, DNS나 이미지를 바꾼 뒤에는 「삭제 후 재구축」을 누르세요.',
      ...
      templateStepHint: '이 단계에서는 클러스터에 이미 있는 템플릿만 나열하며 자동으로 만들지 않습니다. 표준 템플릿이 없으면 「华清智言知识库 표준 템플릿 만들기」를, DNS나 이미지를 바꾼 뒤에는 「삭제 후 재구축」을 누르세요. 템플릿이 준비된 뒤에 계속할 수 있습니다.',
      loadingTemplates: 'Loading templates from the cluster...',
      templateBuildingHint: '표준 템플릿을 구축하는 중입니다. 목록이 자동으로 새로고침됩니다.',
      templateUntaggedHint: '빌드는 끝났지만 default 태그가 붙은 빌드가 없어 샌드박스 생성 시 이 템플릿을 찾을 수 없습니다. E2B에서 템플릿을 삭제하고 새로고침하면 华清智言知识库가 다시 빌드합니다.',
      templateFailedReason: '빌드 실패: {reason}',
      noTemplates: 'No templates were returned by this cluster.',
      weknoraStandardTemplate: '华清智言知识库 표준 템플릿',
      ...
      replaceStandardTemplateConfirm: '현재 설정(DNS 포함)으로 华清智言知识库 표준 템플릿을 다시 만듭니다. 새 템플릿이 준비되기 전에는 기존에 쓸 수 있는 템플릿을 삭제하지 않습니다.',
      ...
      cubeDnsServersHelp: '선택 사항입니다. 华清智言知识库 표준 템플릿에 기록되는 nameserver IP입니다. 비워 두면 클러스터 기본값(보통 119.29.29.29)을 사용합니다. 공용 DNS로 UDP/53이 나가지 않으면 Cube 호스트 /etc/resolv.conf에서 도달 가능한 주소를 넣고, 10/8, 172.16/12, 192.168/16은 피하세요. 이미 있는 표준 템플릿은 카드에서 「재구축」을 눌러야 변경이 적용됩니다.',
    ...
    weknoraCloud: {
      ...
      modelHintConfigured: '클라우드 서비스 자격 증명이 설정되었습니다.',
      ...
      addModelsDisplayName: {
        chat: '클라우드 서비스 대화',
        embedding: '클라우드 서비스 Embedding',
        rerank: '클라우드 서비스 ReRank',
        vllm: '클라우드 서비스 비전'
      },
      modelsSection: {
        title: '클라우드 모델',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '지식 베이스 만들기',
        desc: '모든 것의 시작점입니다. 문서, 웹페이지, FAQ를 업로드하면 华清智言知识库가 자동으로 분석하고 색인합니다. 여기를 클릭해 지식 베이스로 이동하세요.'
      },
      welcome: {
        title: '华清智言知识库에 오신 것을 환영합니다',
