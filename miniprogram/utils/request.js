...
function request(path, options = {}) {
  ...
  if (!settings.baseUrl) {
    return Promise.reject(new Error("Please configure the 华清智言知识库 API base URL first."));
  }
  if (!settings.apiKey) {
    return Promise.reject(new Error("Please configure the 华清智言知识库 API key first."));
  }
